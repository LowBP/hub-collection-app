...
const StateBadge: React.FC<StateBadgeProps> = ({ label, state }) => {
    ...
    return (
        <span className={`absolute top-2 left-2 inline-flex items-center  text-xs font-medium px-2.5 py-0.5 rounded-full ${bgTextClassName}`}>
            <span className={`w-2 h-2 me-1 rounded-full ${dotClassName}`}></span>
            {label}
        </span>
    );
};
